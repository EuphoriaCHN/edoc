...
import AddJsxComponentModal from './AddJSXComponentModal';
    ...
    */
    ...
    React.useEffect(() => {
        ...
        editor.getUI().getToolbar().addButton(new TuiEditor.Button({
            ...
            tooltip: t('插⼊前端组件（Ctrl + M）'),
            event: 'onClickInsertJSXComponent',
            className: ''
        }));
        // const openingEditorThemeSelectButton = $('<button id="edocEditTheme" class="edoc-toolbar-button button-theme tui-toolbar-icons"></button>');
        // editor.getUI().getToolbar().addButton(new TuiEditor.Button({
        ...
        editor.eventManager.listen('prettier', function () {
            try {
                ...
                const res = prettierMDX(content);
                setEditorValue(res);
                message.success(t('格式化成功'));
            } catch (err) {
                message.error(err.message || JSON.stringify(err));
                message.error(t('格式化失败'));
            }
        });
        // 清除默认的图⽚添加 hook
